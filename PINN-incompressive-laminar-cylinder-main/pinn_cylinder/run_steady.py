...
class Net(DeepModel_single):
    ...
    def equation(self, inn_var, out_var):
        ...
        dpdy = dpda[..., 1:2]
        dudx, dudy, dvdx, dvdy = duda[..., 0:1], duda[..., 1:2], dvda[..., 0:1], dvda[..., 1:2]
        ...
        eq_p = p + (s11 + s22) / 2
        eq_u = self.rho * (u*dudx + v*dudy) - s11_1 - s12_2
        eq_v = self.rho * (u*dvdx + v*dvdy) - s12_1 - s22_2
...
if __name__ == '__main__':
    ...
    triang.set_mask(np.hypot(fields_fluent[triang.triangles, 0].mean(axis=1) - 0.2,
                             fields_fluent[triang.triangles, 1].mean(axis=1) - 0.2) < 0.05)
